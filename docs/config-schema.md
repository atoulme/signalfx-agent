...
| `splunkEnabled` | no | bool | Enable logging to a Splunk Enterprise instance (**default:** `false`) |
| `splunkURL` | no | string | URL to Splunk HTTP Event Collector |
| `splunkToken` | no | string | Splunk HTTP Event Collector token |
| `splunkSource` | no | string | Splunk source field value, description of the source of the event |
| `splunkSourceType` | no | string | Splunk source type, optional name of a sourcetype field value |
| `splunkIndex` | no | string | Splunk index, optional name of the Splunk index to store the event in |
| `splunkSkipTLSVerify` | no | bool | Skip verifying the certificate of the HTTP Event Collector (**default:** `false`) |
